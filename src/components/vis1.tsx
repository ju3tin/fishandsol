...
interface GameVisualProps {
  ...
  onCashout: (multiplier: number) => void; // Add this line
  handleCashout: (multiplier: number) => void; // Update this line to accept a multiplier
}
const GameVisual: React.FC<GameVisualProps> = ({ currentMultiplier, onCashout, handleCashout, }) => {
  ...
  useEffect(() => {
    ...
    function animate() {
      ...
      // Call handleCashout directly
      handleCashout(currentMultiplier); // Call the handleCashout function

      // Draw a dot at the fish position
      ctx.fillStyle = "red"; // Color of the dot
      ctx.beginPath();
      ctx.arc(fishPos.x, fishPos.y, 5, 0, Math.PI * 2); // Draw a dot
      ctx.fill();
    ...
    if (gameState5.status === "Running") {
      animate();
       // Call handleCashout with the current multiplier when needed
    // Print the current multiplier before calling handleCashout
   // console.log("Current Multiplier:", currentMultiplier);
   console.log(`i need this to end`+handleCashout(currentMultiplier)); // Call with the multiplier
  ...
  }, [gameState5.status, handleCashout]);
